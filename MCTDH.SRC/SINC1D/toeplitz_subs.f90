...
subroutine circ1d_sub_real(rbigcirc,rmultvector,rffback,totdim,howmany)
  ...
  real*8,intent(in) :: rmultvector(2*totdim,howmany), &
       rbigcirc(2*totdim)
  real*8,intent(out) :: rffback(2*totdim,howmany)
  complex*16 :: multvector(2*totdim,howmany), &              !! AUTOMATIC
       ffback(2*totdim,howmany),   bigcirc(2*totdim)
...
end subroutine circ1d_sub_real
...
subroutine circ1d_sub(bigcirc,multvector,ffback,totdim,howmany)
  ...
  implicit none
  integer,intent(in) :: totdim,howmany
  complex*16,intent(in) ::  bigcirc(2*totdim,1), &
       multvector(2*totdim,howmany)
  complex*16,intent(out) :: ffback(2*totdim,howmany)
  complex*16 :: ffmat(2*totdim),ffvec(2*totdim,howmany),& 
       ffprod(2*totdim,howmany)                            !! AUTOMATIC
  integer :: ii

  ffvec=0; ffmat=0
  call myzfft1d(multvector(:,:),ffvec(:,:),2*totdim,howmany)
  call myzfft1d(bigcirc(:,1),ffmat(:),2*totdim,1)
...
subroutine circ1d_sub_real_mpi(rbigcirc,rmultvector,rffback,dim1,times,howmany)
  ...
  integer :: atime,btime
  real*8,intent(in) :: rmultvector(2*dim1,howmany),   rbigcirc(2*dim1)
  real*8,intent(out) :: rffback(2*dim1,howmany)
  complex*16 :: multvector(2*dim1,howmany), &                    !! AUTOMATIC
        ffback(2*dim1,howmany),  bigcirc(2*dim1)
  ...
  call circ1d_sub_mpi(bigcirc,multvector,ffback,dim1,times,howmany)

  call myclock(atime)
  rffback(:,:)=real(ffback(:,:),8)
  call myclock(btime); times(7)=times(7)+btime-atime

end subroutine circ1d_sub_real_mpi
...
subroutine circ1d_sub_mpi(bigcirc,multvector,ffback,dim1,times,howmany)
  ...
  integer,intent(in) :: dim1,howmany
  integer,intent(inout) :: times(*)
  complex*16,intent(in) ::  multvector(2*dim1,howmany), bigcirc(2*dim1,1)
  complex*16,intent(out) :: ffback(2*dim1,howmany)
  integer :: ii
  integer :: atime,btime,myrank,nprocs
  complex*16 :: ffmat(2*dim1), &                                     !! AUTOMATIC
       ffvec(2*dim1,howmany),  ffprod(2*dim1,howmany)
...
!$OMP DO SCHEDULE(STATIC)
  do ii=1,howmany
     ffprod(:,ii)=ffvec(:,ii)*ffmat(:)/(2*dim1*nprocs)
  enddo
!$OMP END DO
!$OMP END PARALLEL
