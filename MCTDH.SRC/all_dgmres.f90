...
!!$  DJH: these are functions used by dgmres that come from opkda1.f.
!!$   I removed dlsodpk as it was the largest file in the distribution and
!!$   I have odex.f which should be ok if needed.
!!$   Presumably I removed these from the dgmres files.  Keeping these
!!$    not reinstating those to ensure continuity and peace of mind.
...
!!$ DECK DGMRES
      SUBROUTINE DGMRES0(N, B, X, NELT, IA, JA, A, ISYM, MATVEC, MSOLVE, &
...
!!$      .. Array Arguments ..
      DOUBLE PRECISION A(*), B(N), RGWK(LRGW), RWORK(*), SB(N), &
                       SX(N), X(N)
      INTEGER IA(*), IGWK(LIGW), IWORK(*), JA(*)
!!$      .. Subroutine Arguments ..
      EXTERNAL MATVEC, MSOLVE
!!$      .. Local Scalars ..
...
!!$ ------------- LAST LINE OF DGMRES FOLLOWS ----------------------------
      END SUBROUTINE DGMRES0

      SUBROUTINE DGMRES(N, B, X, &
         MATVEC, MSOLVE, &
         ITOL, TOL, ITER, ERR, IERR, IUNIT, RGWK, LRGW, &
         IGWK, LIGW)
!!$      .. Scalar Arguments ..
      DOUBLE PRECISION ERR, TOL
      INTEGER IERR, ISYM, ITER, ITMAX, ITOL, IUNIT, LIGW, LRGW, N, NELT
!!$      .. Array Arguments ..
      DOUBLE PRECISION A(2), RGWK(LRGW), RWORK(2), SB(N), &
                       SX(N)
      INTEGER IA(2), IGWK(LIGW), IWORK(2), JA(2)
!!$      .. Subroutine Arguments ..
      EXTERNAL MATVEC, MSOLVE
#ifdef REALGO
#define XXX 1
...
#define XXX 2
#endif
      DATATYPE b(n/XXX), x(n/XXX)
      real*8 real_b(n), real_x(n), realvec(2)
      DATATYPE cvec(2)
      
      real_b(:) = transfer(b,realvec)
      real_x(:) = transfer(x,realvec)
      CALL DGMRES0(N, REAL_B, REAL_X, NELT, IA, JA, A, ISYM, MATVEC, MSOLVE, &
         ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT, SB, SX, RGWK, LRGW, &
         IGWK, LIGW, RWORK, IWORK)
      b(:) = transfer(real_b,cvec)
      x(:) = transfer(real_x,cvec)
...
!!$      .. Array Arguments ..
!djh      DOUBLE PRECISION A(NELT), B(*), DL(*), HES(MAXLP1,*), Q(*), R0(*), &
      DOUBLE PRECISION A(*), B(*), DL(*), HES(MAXLP1,*), Q(*), R0(*), &
                       RPAR(*), SR(*), SZ(*), V(N,*), WK(*), X(*), &
                       XL(*), Z(*)
!djh      INTEGER IA(NELT), IPAR(*), JA(NELT)
      INTEGER IA(*), IPAR(*), JA(*)
!!$      .. Subroutine Arguments ..
      EXTERNAL MATVEC, MSOLVE
!!$      .. Local Scalars ..
...
!!$ DECK DNRM2
      DOUBLE PRECISION FUNCTION djhDNRM2 (N, DX, INCX)
...
!!$ DECK DORTH
      SUBROUTINE DORTHPAR (VNEW, V, HES, N, LL, LDHES, KMP, SNORMW)
!!$ ***BEGIN PROLOGUE  DORTH
!!$ ***SUBSIDIARY
!!$ ***PURPOSE  Internal routine for DGMRES.
!!$ ***LIBRARY   SLATEC (SLAP)
...
!!$ 
!!$ ***SEE ALSO  DGMRES
!!$ ***ROUTINES CALLED  DAXPY, DDOT, djhDNRM2
!!$ ***REVISION HISTORY  (YYMMDD)
!!$    890404  DATE WRITTEN
!!$    890404  Previous REVISION DATE
!!$    890915  Made changes requested at July 1989 CML Meeting.  (MKS)
...
!!$ DECK DGMRES
      SUBROUTINE DGMRESPAR0 (N, B, X, NELT, IA, JA, A, ISYM,  &
         MATVEC, MSOLVE, &
         ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT, SB, SX, RGWK, LRGW, &
         IGWK, LIGW, RWORK, IWORK)
!!$ ***BEGIN PROLOGUE  DGMRES
...
!!$                   G. F. Carey, Ed., Parallel Supercomputing: Methods,
!!$                   Algorithms and Applications, Wiley, 1989, pp.135-155.
!!$ ***ROUTINES CALLED  D1MACH, DCOPY, djhDNRM2, DPIGMR
!!$ ***REVISION HISTORY  (YYMMDD)
!!$    890404  DATE WRITTEN
!!$    890404  Previous REVISION DATE
!!$    890915  Made changes requested at July 1989 CML Meeting.  (MKS)
...
!!$  DJH 10-2015
!!$  DJH 10-2015
!!$  DJH 10-2015 MAXL can be > N for parallel     LHES = LR + N + 1
!!$  DJH 10-2015
!!$  DJH 10-2015:
...
!!$    ------------------------------------------------------------------
      ...
      CALL DPIGMRPAR(N, RGWK(LR), SB, SX, JSCAL, MAXL, MAXLP1, KMP, &
             NRSTS, JPRE, MATVEC, MSOLVE, NMSL, RGWK(LZ), RGWK(LV), &
             RGWK(LHES), RGWK(LQ), LGMR, RWORK, IWORK, RGWK(LW), &
             RGWK(LDL), RHOL, NRMAX, B, BNRM, X, RGWK(LXL), ITOL, &
             TOL, NELT, IA, JA, A, ISYM, IUNIT, IFLAG, ERR)
...
!!$          Error return.  Inconsistent ITOL and JPRE values.
 650  CONTINUE
      ...
      RETURN
!!$ ------------- LAST LINE OF DGMRES FOLLOWS ----------------------------
      END SUBROUTINE DGMRESPAR0

      SUBROUTINE DGMRESPAR(N, B, X, &
         MATVEC, MSOLVE, &
         ITOL, TOL, ITER, ERR, IERR, IUNIT, RGWK, LRGW, &
         IGWK, LIGW)
!!$      .. Scalar Arguments ..
...
#define XXX 2
      ...
      real_b(:) = transfer(b,realvec)
      real_x(:) = transfer(x,realvec)
      CALL DGMRESPAR0(N, REAL_B, REAL_X, NELT, IA, JA, A, ISYM, MATVEC, MSOLVE, &
         ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT, SB, SX, RGWK, LRGW, &
         IGWK, LIGW, RWORK, IWORK)
      b(:) = transfer(real_b,cvec)
      x(:) = transfer(real_x,cvec)

      end SUBROUTINE DGMRESPAR

!! calls dxlcal-NOT


!!$ DECK ISDGMRPAR
      INTEGER FUNCTION ISDGMRPAR (N, B, X, XL, NELT, IA, JA, A, ISYM, &
         MSOLVE, NMSL, ITOL, TOL, ITMAX, ITER, ERR, IUNIT, R, Z, DZ, &
         RWORK, IWORK, RNRM, BNRM, SB, SX, JSCAL, KMP, LGMR, MAXL, &
         MAXLP1, V, Q, SNORMW, PROD, R0NRM, HES, JPRE)
!!$ ***BEGIN PROLOGUE  ISDGMRPAR
!!$ ***SUBSIDIARY
!!$ ***PURPOSE  Generalized Minimum Residual Stop Test.
!!$             This routine calculates the stop test for the Generalized
!!$             Minimum RESidual (GMRES) iteration scheme.  It returns a
...
!!$ ***LIBRARY   SLATEC (SLAP)
!!$ ***CATEGORY  D2A4, D2B4
!!$ ***TYPE      DOUBLE PRECISION (ISSGMR-S, ISDGMRPAR-D)
!!$ ***KEYWORDS  GMRES, LINEAR SYSTEM, SLAP, SPARSE, STOP TEST
!!$ ***AUTHOR  Brown, Peter, (LLNL), pnbrown@llnl.gov
!!$            Hindmarsh, Alan, (LLNL), alanh@llnl.gov
!!$            Seager, Mark K., (LLNL), seager@llnl.gov
...
!!$       EXTERNAL MSOLVE
!!$ 
!!$       IF (ISDGMRPAR(N, B, X, XL, NELT, IA, JA, A, ISYM, MSOLVE,
!!$      $     NMSL, ITOL, TOL, ITMAX, ITER, ERR, IUNIT, R, Z, DZ,
!!$      $     RWORK, IWORK, RNRM, BNRM, SB, SX, JSCAL,
!!$      $     KMP, LGMR, MAXL, MAXLP1, V, Q, SNORMW, PROD, R0NRM,
!!$      $     HES, JPRE) .NE. 0) THEN ITERATION DONE
...
!!$      This routine does not verify that ITOL has a valid value.
!!$      The calling routine should make such a test before calling
!!$      ISDGMRPAR, as is done in DGMRES.
!!$ 
!!$ ***SEE ALSO  DGMRES
!!$ ***ROUTINES CALLED  D1MACH, DCOPY, djhDNRM2, DRLCAL, DSCAL, DXLCAL
!!$ ***COMMON BLOCKS    DSLBLK
!!$ ***REVISION HISTORY  (YYMMDD)
!!$    890404  DATE WRITTEN
!!$    890404  Previous REVISION DATE
...
!!$      .. Save statement ..
      SAVE SOLNRM
!!$ ***FIRST EXECUTABLE STATEMENT  ISDGMRPAR
      ISDGMRPAR = 0
...
!!$ 
         IF ((JSCAL .EQ. 0) .OR.(JSCAL .EQ. 2)) THEN
!!$          err = ||x-TrueSolution||/||TrueSolution||(2-Norms).
            IF ( ITER.EQ.0 ) SOLNRM = djhDNRM2(N, SOLN, 1)
            DO 30 I = 1, N
               DZ(I) = XL(I) - SOLN(I)
 30         CONTINUE
...
!!$ 
 ...
 1020 FORMAT(1X,' ITER = ',I5, ' IELMAX = ',I5, &
           ' |R(IELMAX)/X(IELMAX)| = ',D12.5)
!!$ ------------- LAST LINE OF ISDGMRPAR FOLLOWS ----------------------------
      END FUNCTION


!!$ DECK DPIGMR
      SUBROUTINE DPIGMRPAR (N, R0, SR, SZ, JSCAL, MAXL, MAXLP1,  &
         KMP, NRSTS, &
         JPRE, MATVEC, MSOLVE, NMSL, Z, V, HES, Q, LGMR, RPAR, IPAR, WK, &
         DL, RHOL, NRMAX, B, BNRM, X, XL, ITOL, TOL, NELT, IA, JA, A, &
         ISYM, IUNIT, IFLAG, ERR)
...
!!$ 
!!$ ***SEE ALSO  DGMRES
!!$ ***ROUTINES CALLED  DAXPY, DCOPY, DHELS, DHEQR, djhDNRM2, DORTH, DRLCAL,
!!$                     DSCAL, ISDGMRPAR
!!$ ***REVISION HISTORY  (YYMMDD)
!!$    890404  DATE WRITTEN
!!$    890404  Previous REVISION DATE
...
!!$    -------------------------------------------------------------------
 ...
 20      CONTINUE
      ...
      ITER = NRSTS*MAXL
!!$ 
!!$          Call stopping routine ISDGMRPAR.
!!$ 
      IF (ISDGMRPAR(N, B, X, XL, NELT, IA, JA, A, ISYM, MSOLVE, &
          NMSL, ITOL, TOL, ITMAX, ITER, ERR, IUNIT, V(1,1), Z, WK, &
          RPAR, IPAR, R0NRM, BNRM, SR, SZ, JSCAL, &
...
!!$    -------------------------------------------------------------------
 ...
 65        CONTINUE
        ...
        CALL DORTHPAR(V(1,LL+1), V, HES, N, LL, MAXLP1, KMP, SNORMW)
