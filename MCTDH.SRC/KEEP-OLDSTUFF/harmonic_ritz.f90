...
!! DOES PARTIAL BLOCK IF AT THE END (leaving this)
     ...
     if (iorder.eq.1) then
        ...
        call inh0multsub(thislanblocknum,lanvects(:,:,iorder-1),lanvects(:,:,iorder))
     endif

     call inh0multsub(thislanblocknum,lanvects(:,:,iorder),lansimplemultvects(:,:,iorder))
...
subroutine harm_orthog_oneblock(invects, &
  ...
  call inh0multsub(lanblocknum,invects(:,:),lansimplemultvects(:,:))
...
!!TEMP?    call hermsqrtmatsmooth(lantransinv,lantrans,lanblocknum,lanblocknum,0d-10)

!!TEMP?:
!  lantrans(:,:)=lantransinv(:,:)
  call cnormorthogmat(lantransinv,lanblocknum)
...
!! check
  ...
  do i=1,lanblocknum
     do j=1,lanblocknum
        if (abs(tempdots(i,j)).gt.1d-8) then
           print *, "AACK DOTS:"
           do k=1,lanblocknum
