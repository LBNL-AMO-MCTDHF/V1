...
!!$ ---     uniform rational Chebyshev approximation ...
         ...
         enddo
         wsp(iexph) = 1.0d0
         call DNCHBV(mx,sgn*t_step,wsp(ih),mh,wsp(iexph))  !$$ DJH,wsp(ifree+mx))
      endif
...
!!$ 
      ...
      enddo
      hnorm = ABS( t*hnorm )
!!$ ! djh      if ( hnorm.eq.0.0d0 ) stop 'Error - null H in input of DGPADM.'
!!$ ! djh:
      if ( hnorm.eq.0.0d0 ) then
!         print *,  'djh - null H in input of DGPADM.'
         do i=1,m
            do j=1,m
               if (i==j) then
                  wsp(j + (i-1)*m) = 1.d0
...
!!$ 
      ...
      if ( iflag.ne.0 ) then
         print *, 'Problem in DGESV (within DGPADM)',iflag
         stop
      endif
...
!!$ 
      ...
      enddo
      hnorm = ABS( t*hnorm )
!!$ !djh      if ( hnorm.eq.0.0d0 ) stop 'Error - null H in input of ZGPADM.'
!!$ ! djh:
      if ( hnorm.eq.0.0d0 ) then
!!         print *,  'djh - null H in input of ZGPADM.'
         do i=1,m
            do j=1,m
               if (i==j) then
                  wsp(j + (i-1)*m) = 1.d0
...
!!$ ----------------------------------------------------------------------|
      ...
      subroutine DGCHBVQ( m, t, H,ldh, y, iflag )
      implicit none
      integer          m, ldh, iflag
      double precision t, H(ldh,m)
#ifdef REALGO
#define XXX 1
#else
#define XXX 2
#endif
      DATATYPE y(m/XXX)
      real*8 real_y(m), realvec(2)
      DATATYPE cvec(2)
      
      real_y(:) = transfer(y,realvec)
      call DGCHBV( m, t, H,ldh, real_y, iflag )
      y(:) = transfer(real_y,cvec)

      end subroutine DGCHBVQ

!!$ DJH      subroutine DGCHBV( m, t, H,ldh, y, wsp, iwsp, iflag )
      subroutine DGCHBV( m, t, H,ldh, y, iflag )
      implicit none
      integer          m, ldh, iflag, iwsp(m)  !! AUTOMATIC
      double precision t, H(ldh,m), y(m)
      complex*16       wsp(m*(m+2))  !! AUTOMATIC
...
!!$      subroutine DSCHBV( m, t, H,ldh, y, wsp, iwsp, iflag )
      subroutine DSCHBV( m, t, H,ldh, y, iflag )
      implicit none
      integer          m, ldh, iflag, iwsp(m)   !! AUTOMATIC
      double precision t, H(ldh,m), y(m)
      complex*16       wsp(m*(m+2))       !! AUTOMATIC
...
!!$ ----------------------------------------------------------------------|
!!$ ----------------------------------------------------------------------|
!!$ DJH      subroutine ZGCHBV( m, t, H,ldh, y, wsp, iwsp, iflag )
      subroutine ZGCHBV( m, t, H,ldh, y, iflag )
      implicit none
      integer          m, ldh, iflag, iwsp(m)  !! AUTOMATIC
      double precision t
      complex*16       H(ldh,m), y(m), wsp(m*(m+2))  !! AUTOMATIC
...
!!$ ----------------------------------------------------------------------|
      ...
      integer          m, ldh
      double precision t, H(ldh,m), y(m)
      complex*16       wsp(m*(m+2))   !! AUTOMATIC DJH
...
!!$ ----------------------------------------------------------------------|
      ...
      call DGEXPVxxx( n, m, t, v, w, tol, anorm,  &
              wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,xx)
      end SUBROUTINE

      subroutine DGEXPVxxx( n, m, t, v, w, tol, anorm,  &
          wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,stepguess)
...
!!$ 
      ...
      intrinsic AINT,ABS,DBLE,LOG10,MAX,MIN,NINT,SIGN,SQRT
      double precision DDOT, DNRM2,stepguess
...
!!! djh don't decrease... not sure if it would
...
!!$ ----------------------------------------------------------------------|
      ...
      subroutine DGEXPVxxx20( n, m, t, v, w, tol, anorm,  &
          wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,stepguess,  &
          dotsub,maxdim)
      implicit none
      integer n,m, lwsp, liwsp, itrace, iflag, iwsp(liwsp),fileptr
      double precision t, tol, anorm, v(n), w(n), wsp(lwsp)
      external matvec,dotsub
...
!!$ >>>  break_tol = anorm*tol
      ...
      sgn = SIGN( 1.0d0,t )
      call DCOPY( n, v,1, w,1 )
!!djhpar      beta = DNRM2( n, w,1 )

      call dotsub( w,w,n ,beta)
      beta=sqrt(beta)
!!
      vnorm = beta
      hump = beta 
...
!! djhpar            hij = DDOT( n, wsp(iv+(i-1)*n),1, wsp(j1v),1 )
            call dotsub( wsp(iv+(i-1)*n),wsp(j1v),n,hij )
!!
            call DAXPY( n, -hij, wsp(iv+(i-1)*n),1, wsp(j1v),1 )
            wsp(ih+(j-1)*mh+i-1) = hij
         enddo

!!djhpar         hj1j = DNRM2( n, wsp(j1v),1 )
          call dotsub( wsp(j1v), wsp(j1v), n, hj1j )
          hj1j=sqrt(hj1j)
!!
...
!!djhpar      avnorm = DNRM2( n, wsp(j1v),1 )
      call dotsub(wsp(j1v),wsp(j1v),n,avnorm)
      avnorm=sqrt(avnorm)
!!
...
!!$ 
      if ( k1.eq.0 ) then
         ...
         if ( p1.gt.10.0d0*p2 ) then
            err_loc = p2
            xm = 1.0d0/DBLE( m )
!! DJH GOSH THIS LOOKS STRANGE     - WAAAAAY BETTER WITHOUT!! SUPPOSED TO BE GEOMETRIC AVG??? (plus sign)
!         elseif ( p1.gt.p2 ) then
!            err_loc = (p1*p2)/(p1-p2)
!            xm = 1.0d0/DBLE( m )
         else
...
!!$ 
 ...
 500  continue
      ...
      END SUBROUTINE
      subroutine DGEXPVxxx2( n, m, t, v, w, tol, anorm,  &
          wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,stepguess,  &
          dotsub,maxdim)
      implicit none
      integer n,m, lwsp, liwsp, itrace, iflag, iwsp(liwsp),fileptr
      double precision t, tol, anorm, wsp(lwsp)
#ifdef REALGO
#define XXX 1
#else
#define XXX 2
#endif
      DATATYPE v(n/XXX), w(n/XXX)
      real*8 :: real_v(n), real_w(n)      
      external matvec,dotsub
      integer maxdim
      double precision stepguess
      real*8 realvec(2)
      ...
      real_v(:) = transfer(v,realvec)
      real_w(:) = transfer(w,realvec)
      call DGEXPVxxx20( n, m, t, real_v, real_w, tol, anorm,  &
          wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,stepguess,  &
          dotsub,maxdim)
      v(:) = transfer(real_v,cvec)
      w(:) = transfer(real_w,cvec)

      end subroutine DGEXPVxxx2
...
!!$ ----------------------------------------------------------------------|
      subroutine ZGEXPVxxx( n, m, t, v, w, tol, anorm,  &
          wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,stepguess)
...
!!$ ----------------------------------------------------------------------|
      ...
      double precision sgn, t_out, tbrkdwn, step_min,step_max, err_loc,  &
                      s_error, x_error, t_now, t_new, t_step, t_old,  &
                      xm, beta, break_tol, p1, p2, p3, eps, rndoff,  &
                      vnorm, avnorm, hj1j, hump, SQR1,t_last
      complex*16 hij
      intrinsic AINT,ABS,DCMPLX,DBLE,INT,LOG10,MAX,MIN,NINT,SIGN,SQRT
!!$ ! djh      complex*16 ZDOTC
      double precision DZNRM2, stepguess
...
!!$ 
      ...
      do 200 j = 1,m
         ...
         call matvec( wsp(j1v-n), wsp(j1v) )
         do i = 1,j
!djh            hij = ZDOTC( n, wsp(iv+(i-1)*n),1, wsp(j1v),1 )
            hij=DOT_PRODUCT(wsp(iv+(i-1)*n:iv+i*n-1), wsp(j1v:j1v+n-1))
...
!!$ ! djh         hjj = ZDOTC( n, wsp(j1v-n),1, wsp(j1v),1 )
         hjj = DOT_PRODUCT( wsp(j1v-n:j1v-1), wsp(j1v:j1v+n-1))
...
!!$ ----------------------------------------------------------------------|
      ...
      subroutine DGPHIV0( n, m, t, u, v, w, tol, anorm,  &
...
!      if ( iflag.ne.0 ) stop 'bad sizes(in input of DGPHIV)'
      if ( iflag.ne.0 ) then
         print *, 'bad sizes (in input of DGPHIV)', iflag
         print *, m,n
...
!! DJH : HAS NO eFFECT with spfs.  NEver comes close, criterion ~1d2
      break_tol = 1.0d-7
!!$ >>>  break_tol = tol
!!$ >>>  break_tol = anorm*tol
...
!!$ ---  error estimate ...
      if ( k1.eq.0 ) then
         ...
         if ( p1.gt.10.0d0*p2 ) then
            err_loc = p2
            xm = 1.0d0/DBLE( m+1 )
!! DJH GOSH THIS LOOKS STRANGE.  if avnorm is just less than 1 one it will never converge
!         elseif ( p1.gt.p2 ) then
!            err_loc = (p1*p2)/(p1-p2)
!            xm = 1.0d0/DBLE( m+1 )
         else
...
!!$ 
      ...
      p1 = 10.0d0**(NINT( LOG10( t_new )-SQR1 )-1)
      t_new = AINT( t_new/p1 + 0.55d0 ) * p1 
!      print *, "       NEW,STEP",t_new,t_step

!! DJH don't decrease here..  not sure if it would
      if (t_new.lt.t_step) then
         t_new = t_step
      endif
...
#define XXX 2
#endif
      DATATYPE u(n/XXX), v(n/XXX), w(n/XXX)
      real*8 :: real_u(n), real_v(n), real_w(n)
      external matvec
      real*8 realvec(2)
      DATATYPE cvec(2)

      real_u(:) = transfer(u,realvec)
      real_v(:) = transfer(v,realvec)
      real_w(:) = transfer(w,realvec)
      call DGPHIV0( n, m, t, real_u, real_v, real_w, tol, anorm,  &
       wsp,lwsp, iwsp,liwsp,matvec,itrace,iflag,fileptr,guess_stepsize) 

      u(:) = transfer(real_u,cvec)
      v(:) = transfer(real_v,cvec)
      w(:) = transfer(real_w,cvec)
...
!! PARALLEL SUBROUTINE.
      ...
      subroutine DGPHIVxxx20( n, m, t, u, v, w, tol, anorm,  &
       wsp,lwsp, iwsp,liwsp,matvec,itrace,iflag,fileptr,guess_stepsize,  &
       dotsub,maxdim) 
      implicit none
      integer n, m, lwsp, liwsp, itrace, iflag, iwsp(liwsp),fileptr
...
!!$ -----The following parameters may also be adjusted herein-------------|
!!$ 
      integer mxstep, mxreject, ideg,maxdim
      double precision delta, gamma
      parameter( mxstep   = 1000,   &
                mxreject = 0,  &
                ideg     = 6,   &
...
!!DJHPAR!!      if ( m.ge.n .or. m.le.0 ) iflag = -3
      ...
      if ( iflag.ne.0 ) then
         print *, 'bad sizes (in input of DGPHIVxxx2)', iflag
         print *, m,n,maxdim
         print *, lwsp,n*(m+3)+5*(m+3)**2+ideg+1 
         print *, liwsp, m+3 
         stop 
...
!!DJHPAR      beta = DNRM2( n, wsp(iv),1 )
      call dotsub(wsp(iv),wsp(iv),n,beta)
      beta=sqrt(beta)
...
#define XXX 2
      ...
      real_v(:) = transfer(v,realvec)
      real_w(:) = transfer(w,realvec)
      call DGPHIVxxx20( n, m, t, real_u, real_v, real_w, tol, anorm,  &
       wsp,lwsp, iwsp,liwsp,matvec,itrace,iflag,fileptr,guess_stepsize,  &
       dotsub,maxdim) 
      u(:) = transfer(real_u,cvec)
      v(:) = transfer(real_v,cvec)
      w(:) = transfer(real_w,cvec)
      end subroutine DGPHIVxxx2
...
!! djh adapted dgexpv.  computes 2/3 + 1/3 exp(t*A)*v
!!    does not constrain 2/3 + 1/3 exp(t*A) to be unitary; commented out

      subroutine DGEXPTHIRD0( n, m, t, v, w, tol, anorm,  &
          wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,  &
          guess_stepsize)
...
!!$ ---  check restrictions on input parameters ...
      ...
      if ( liwsp.lt.m+2 ) iflag = -2
      if ( m.ge.n .or. m.le.0 ) iflag = -3
      if ( iflag.ne.0 ) stop 'bad sizes (in input of DGEXPTHIRD)'
!!$ 
!!$ ---  initialisations ...
!!$ 
! djh
...
!! djh constrain (2/3 + 1/3 exp(t H)) v  to be unitary (no, is commented out, not constrained)
!!      sum=ddot(mx,wsp(iexph),1,wsp(iexph),1)
!!      print *, "SUM IS ", sum
!!      stop
...
#define XXX 2
      ...
      call DGEXPTHIRD0( n, m, t, real_v, real_w, tol, anorm,  &
          wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,  &
          guess_stepsize)
      ...
      end subroutine DGEXPTHIRD


!! PARALLEL SUBROUTINE

      subroutine DGEXPTHIRDxxx20( n, m, t, v, w, tol, anorm,  &
          wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,  &
          guess_stepsize,dotsub,maxdim)
      implicit none
      integer n, m, lwsp, liwsp, itrace, iflag, iwsp(liwsp),fileptr
      real*8 guess_stepsize
      double precision t, tol, anorm, v(n), w(n), wsp(lwsp)
...
!!DJHPAR!!      if ( m.ge.n .or. m.le.0 ) iflag = -3
      if ( m.ge.maxdim .or. m.le.0 ) iflag = -3
      if ( iflag.ne.0 ) stop 'bad sizes (in input of DGEXPTHIRDxxx2)'
!!$ 
!!$ ---  initialisations ...
!!$ 
! djh
...
!!$ ---     uniform rational Chebyshev approximation ...
         ...
         enddo
         wsp(iexph) = 1.0d0
         call DNCHBV(mx,sgn*t_step,wsp(ih),mh,wsp(iexph:iexph+mx-1))  !$$ DJH,wsp(ifree+mx))
      endif
...
!! djh 07-2015
      ...
      END subroutine DGEXPTHIRDxxx20

      subroutine DGEXPTHIRDxxx2( n, m, t, v, w, tol, anorm,  &
          wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,  &
          guess_stepsize,dotsub,maxdim)
      implicit none
      integer n, m, lwsp, liwsp, itrace, iflag, iwsp(liwsp),fileptr
...
#define XXX 2
      ...
      call DGEXPTHIRDxxx20( n, m, t, real_v, real_w, tol, anorm,  &
          wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag,fileptr,  &
          guess_stepsize,dotsub,maxdim)
