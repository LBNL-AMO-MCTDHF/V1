...
C  Check error conditions 1 and 2.
     ...
     +   (L3-ABS(M3)+EPS.LT.ZERO))THEN
         IER=1
cdjh         CALL XERMSG('SLATEC','DRC3JJ','L2-ABS(M2) or L3-ABS(M3) '//
cdjh     +      'less than zero.',IER,1)
         RETURN
      ELSEIF((MOD(L2+ABS(M2)+EPS,ONE).GE.EPS+EPS).OR.
     +   (MOD(L3+ABS(M3)+EPS,ONE).GE.EPS+EPS))THEN
         IER=2
cdjh         CALL XERMSG('SLATEC','DRC3JJ','L2+ABS(M2) or L3+ABS(M3) '//
cdjh     +      'not integer.',IER,1)
         RETURN
      ENDIF
C
C
...
C  Check error condition 3.
      IF(MOD(L1MAX-L1MIN+EPS,ONE).GE.EPS+EPS)THEN
         IER=3
cdjh         CALL XERMSG('SLATEC','DRC3JJ','L1MAX-L1MIN not integer.',IER,1)
         RETURN
      ENDIF
...
C  Check error condition 4.
      IER=4
cdjh      CALL XERMSG('SLATEC','DRC3JJ','L1MIN greater than L1MAX.',IER,1)
      RETURN
C
...
cdjh      CALL XERMSG('SLATEC','DRC3JJ','Dimension of result array for '//
cdjh     +            '3j coefficients too small.',IER,1)
      RETURN
C
C
