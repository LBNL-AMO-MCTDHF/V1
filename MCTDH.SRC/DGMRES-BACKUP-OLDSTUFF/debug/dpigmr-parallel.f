...
C
C***SEE ALSO  DGMRES
C***ROUTINES CALLED  DAXPY, DCOPY, DHELS, DHEQR, djhDNRM2, DORTH, DRLCAL,
C                    DSCAL, ISDGMRPAR
...
C     .. Scalar Arguments ..
     ...
     +        MAXLP1, N, NELT, NMSL, NRMAX, NRSTS,myrank
C     .. Array Arguments ..
...
C     .. External Functions ..
      DOUBLE PRECISION djhDNRM2, blah
      INTEGER ISDGMRPAR
      EXTERNAL djhDNRM2, ISDGMRPAR
...
cc         print *, "PIGMR solve ", myrank

         CALL MSOLVE(N, WK, R0, NELT, IA, JA, A, ISYM, RPAR, IPAR)
cc         print *, "PIGMR solvedone ", myrank
 ...
 20      CONTINUE
      ENDIF
cc      print *, "NORM ", myrank
      R0NRM = djhDNRM2(N, V, 1)
cc      print *, "DONENORM", myrank,R0NRM

ccc      call openfile()
ccc      write(971,*) djhdjh," CHECK  djhDNRM2 R0NRM: ", R0NRM
ccc      call closefile()
...
ccc      call openfile()
ccc      write(971,*) djhdjh," CHECK  djhDNRM2 TEM ", TEM
ccc      call closefile()

cc      print *, " CHECK  djhDNRM2 TEM ", TEM, myrank
...
ccc        write(971,*) "  wk(1) check ", wk(1)

        print *, "DDD CHECK ", myrank

        blah=djhDNRM2(N,WK,1)
        print *, blah, "XXX ", myrank
...
ccc        write(971,*) " v(1,ll+1)  check ", ll, v(1,ll+1)

cc        print *, "DDD2 CHECK", myrank
...
ccc        write(971,*) " v(1,ll+1)  check XX ", ll, v(1,ll+1)

        print *, "DORTH", myrank
        CALL DORTHPAR(V(1,LL+1), V, HES, N, LL, MAXLP1, KMP, SNORMW)
        print *, "DORRTHDONE", myrank

ccc      call openfile()
ccc      write(971,*) djhdjh," CHECK  djhDNRM2 SNORMW: ", SNORMW
ccc      call closefile()
...
C   -------------------------------------------------------------------
        ...
        print *, "PROD ", rho,prod,r0nrm,Q(2*LL),LL
 ...
 80        CONTINUE
        ...
        ENDIF
        RHOL = RHO
!        open(779,position="append")
!        write(779,*) "RHOL IS", rhol
!        close(779)
...
C   -------------------------------------------------------------------
        ...
        print *, "STOPCHECK ", myrank,rhol
        IF (ISDGMRPAR(N, B, X, XL, NELT, IA, JA, A, ISYM, MSOLVE,
     $      NMSL, ITOL, TOL, ITMAX, ITER, ERR, IUNIT, DL, Z, WK,
     $      RPAR, IPAR, RHOL, BNRM, SR, SZ, JSCAL,
     $      KMP, LGMR, MAXL, MAXLP1, V, Q, SNORMW, PROD, R0NRM,
     $      HES, JPRE) .NE. 0) then
           print *, "STOPCHECK ED BOY GO", myrank
           GO TO 200

        endif
!        print *, "STOPCHECKKD ", myrank,ll,maxl
...
ccc      call openfile()
ccc      write(971,*) djhdjh," CHECK  djhDNRM2 TEM AGAIN: ", TEM
ccc      write(971,*)
ccc      call closefile()

!!        print *, "CONTNPOW", myrank
...
!!      print *, "ERGO ", myrank
 ...
 100  CONTINUE
!!      print *, "SSUOOGO", myrank
      IF (RHO .LT. R0NRM) GO TO 150
 !!     print *, "ARGO", myrank
 120  CONTINUE
      print *, "IFLAG IS TWO ", myrank
      IFLAG = 2
C
...
!!      print *, "RETURNXX ", myrank

      RETURN

      print *, "IFLAG IS ONE ", myrank
