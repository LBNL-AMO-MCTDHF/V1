...
C***TYPE      DOUBLE PRECISION (ISSGMR-S, ISDGMRPAR-D)
...
C      EXTERNAL MSOLVE
C
C      IF (ISDGMRPAR(N, B, X, XL, NELT, IA, JA, A, ISYM, MSOLVE,
...
C     ISDGMRPAR, as is done in DGMRES.
C
C***SEE ALSO  DGMRES
C***ROUTINES CALLED  D1MACH, DCOPY, djhDNRM2, DRLCAL, DSCAL, DXLCAL
...
C     .. Save statement ..
      SAVE SOLNRM
C***FIRST EXECUTABLE STATEMENT  ISDGMRPAR
      ISDGMRPAR = 0
...
C
         print *, "RNRM,BNRM",RNRM,BNRM,myrank
...
!            call getmyrank(myrank)
!            call openfile()
!            open(986,file="tempout",status="unknown", position="append")
!            write(986,*) "ERR IS ", err, myrank
!            close(986)
!            call mpistop()
...
C------------- LAST LINE OF ISDGMRPAR FOLLOWS ----------------------------
      END
