...
subroutine cooleytukey3d_outofplace_backward_mpi(BLOCKVARS,intranspose,out,dim1,pf,proclist,localnprocs,localrank,howmany)
  implicit none
  ...
  call cooleytukey3d_outofplaceinput_mpi(BLOCKVARS,intransconjg,outconjg,dim1,pf,proclist,localnprocs,localrank,howmany)
...
recursive subroutine cooleytukey3d_outofplace_mpi(BLOCKVARS,in,outtrans,dim1,pf,proclist,localnprocs,localrank,howmany)
  use ct_fileptrmod
  implicit none
  ...
  if (depth.eq.1) then
     ...
     call cooleytukey3d_outofplace_mpi(BLOCKVARS,outtemp,outtrans,dim1,newpf,newproclist,depth,newrank,howmany)
  endif
...
recursive subroutine cooleytukey3d_outofplaceinput_mpi(BLOCKVARS,intranspose,out,dim1,pf,proclist,localnprocs,localrank,howmany)
  use ct_fileptrmod
  implicit none
  integer, intent(in) :: BLOCKVARS,dim1,localnprocs,localrank,howmany,pf(MAXFACTORS),proclist(localnprocs/pf(1),pf(1))
  complex*16, intent(in) :: intranspose(BLOCKPROD,dim1,howmany)
  ...
  if (depth.eq.1) then
     ...
     newpf(1:MAXFACTORS-1)=pf(2:MAXFACTORS); newpf(MAXFACTORS)=1
     call cooleytukey3d_outofplaceinput_mpi(BLOCKVARS,intranspose,temptrans,dim1,newpf,newproclist,depth,newrank,howmany)
  endif
